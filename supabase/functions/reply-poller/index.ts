  ...
  try {
    ...
    for (const userId of userIds) {
      ...
      try {
        ...
        console.log(`User ${userId} has ${sentEmails.length} sent emails.`);

        const refreshToken = await decrypt(tokenData.encrypted_refresh_token, Deno.env.get('ENCRYPTION_KEY')!);
        const accessToken = await getAccessToken(refreshToken);

        for (const email of sentEmails) {
          console.log(`Checking thread for email: ${email.id}, thread: ${email.thread_id}`);
          ...
          if (thread.messages.length > originalMessageCount) {
            console.log(`Thread ${email.thread_id} has new replies.`);
            ...
            for (const message of newMessages) {
              ...
              if (isReplyFromRecipient) {
                ...
                if (insertError) {
                  ...
                  // Follow-up logic removed. Now handled by followup-processor function.
                }
              }
            }
          }
